...
def flush_active_session(
    ...
    *,
    ...
    with fts.cursor() as conn:
        ...
        flush_end = existing.flush_end if existing and existing.flush_end else None
        window_start = flush_end if flush_end and flush_end > session_start else session_start
        if now <= window_start:
            return None
...
# Minimum total dwell (seconds) for a surface to be worth surfacing to the
...
def _format_attention_trajectory(blocks: list[timeline_store.TimelineBlock]) -> str:
    """A dwell-ranked "where the user's attention went" block for the reducer.

    Aggregates the per-block dominant loci (Step 1) into total dwell per surface
    and lists the longest first — a hint about what mattered in the window. Returns
    ``""`` when no surface has meaningful dwell (e.g. blocks predating the locus
    pipeline carry an empty ``attention_surface``), so the prompt is byte-identical
    to before until the column is populated. Self-describing — no separate prompt
    guidance needed.
    """
    totals: dict[str, int] = {}
