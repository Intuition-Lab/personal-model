...
| `dedup_interval_seconds` | 1.0 | Repeated non-focus events on one surface are dropped. The complementary `AXApplicationActivated` + `AXFocusedWindowChanged` pair also collapses; repeated same-type focus notifications continue to the content gate because they may be different real windows. |
