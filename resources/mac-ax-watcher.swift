...
// MARK: - Electron webcontents AX activation (issue #556)
//
// Chromium-based apps only render their webcontents DOM into the macOS AX
// tree while they believe an assistive client is present; otherwise the tree
// is just the native shell (zero `a11y_*` semantic classes) and the per-app
// parser correctly declines — the 65% Feishu miss rate of #548/#553.
//
// Live-probe findings on Feishu (com.electron.lark, 2026-06-12):
//   * `AXManualAccessibility` (Electron's official knob) is NOT supported by
//     Feishu's build — set returns .attributeUnsupported (-25205). We still
//     try it first for forward compatibility with newer Electron runtimes.
//   * `AXEnhancedUserInterface` IS honored: flipping it false→true restores
//     the DOM-derived subtree within ~1s (measured 250 nodes / 0 semantic
//     ↔ 1430 nodes / 361 semantic). It can decay (Chromium resets it), so a
//     one-shot set is not enough — we re-assert on every app-activated event,
//     guarded by a cheap read so the steady state is a single AX read.
//   * The set call returns .cannotComplete (-25208) even when it works
//     (Chromium applies it asynchronously) — error codes are reported in the
//     activation event for observability but never treated as failure.
//
// Scope is deliberately the Feishu main client only (the one app whose parser
// needs the DOM); blanket activation of every Electron app (VSCode etc.)
// would grow their AX trees for no consumer.

let electronAXActivationBundles: Set<String> = ["com.electron.lark"]

/// Best-effort read of `AXEnhancedUserInterface`. Returns false when the
/// attribute is unreadable so activation errs on the side of (re-)asserting.
func isEnhancedUserInterfaceEnabled(_ appElement: AXUIElement) -> Bool {
    var ref: CFTypeRef?
    let err = AXUIElementCopyAttributeValue(
        appElement, "AXEnhancedUserInterface" as CFString, &ref
    )
    guard err == .success, let num = ref as? NSNumber else { return false }
    return num.boolValue
}

/// Force-enable an app's accessibility tree. Two tiers:
///  • EVERY app gets the SAFE `AXManualAccessibility` toggle — Electron/Chromium (Feishu/Lark,
///    Slack, VS Code, many chat clients) ship AX OFF for performance and only turn it on in the
///    presence of assistive tech that watches this attribute. It's a no-op on native apps and has
///    no layout side-effects. The long-lived watcher re-asserts it on focus so Electron AX stays on
///    between the short-lived `mac-ax-helper`'s per-capture reads.
///  • Vetted bundles (`electronAXActivationBundles`) ALSO get the stronger `AXEnhancedUserInterface`
///    (which CAN perturb some apps' layout/AX, hence the allowlist) + an `_electron_ax_activated`
///    telemetry event. The broad manual set stays silent to keep the capture stream clean.
func activateElectronAX(pid: pid_t, name: String, bundleId: String, reason: String) {
    ...
    // Stronger toggle + telemetry stay scoped to vetted bundles.
    guard electronAXActivationBundles.contains(bundleId) else { return }
    if isEnhancedUserInterfaceEnabled(appElement) { return }
    ...
    writer.write(event: [
        ...
        "details": [
            "reason": reason,
            "set_manual_err": manualErr.rawValue,
            "set_enhanced_err": enhancedErr.rawValue,
        ],
    ])
}
...
final class ObserverManager {
    ...
    func addObserver(for pid: pid_t, name: String, bundleId: String) {
        // First sight of an Electron bundle (startup enumeration or app
        // launch): enable webcontents AX exposure before captures need it.
        // Outside the lock — the call does AX round-trips to the target app.
        activateElectronAX(pid: pid, name: name, bundleId: bundleId, reason: "first_seen")
...
func observerCallback(
    ...
    // Re-assert Electron webcontents AX exposure when the user switches to
    // the app — `AXEnhancedUserInterface` can decay (Chromium resets it), and
    // activation is the moment right before captures of this app's content.
    // Idempotent: a single AX read when already enabled.
    if notificationStr == kAXApplicationActivatedNotification as String {
        activateElectronAX(
            pid: context.pid, name: context.name, bundleId: context.bundleId,
